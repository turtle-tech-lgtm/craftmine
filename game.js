/** Minecraft Scroller - v13.0 (Skeleton Added) */
window.startGame=()=>{
    const nameInput = document.getElementById('player-name');
    gameState.playerName = nameInput.value.trim().toUpperCase() || 'STEVE';
...
window.showLeaderboard=()=>{
    const scores = JSON.parse(localStorage.getItem('mc_scores_v2') || '[]');
    const list = document.getElementById('leaderboard-list');
    list.innerHTML = scores.map((s,i) => `<div class="leaderboard-entry"><span>${i+1}. ${s.name}</span><span style="color: #ffff55;">${s.time.toFixed(2)}s</span></div>`).join('') || 'No scores yet!';
    document.getElementById('main-menu').style.display='none';
...
let gameState={level:1,coins:0,hp:3,totalTime:0,elapsedTime:0,startTime:0,isPaused:false,gameOver:false,shieldActive:false,lastFired:0,isStarted:false,isBossLevel:false,dragonHp:10,invuln:false,playerName:'STEVE'};
...
function create(){
    const scene=this;
    const colors={dirt:'#5d3a1a',grass:'#3e8914',shirt:'#00a8a8',pants:'#3c44aa',skin:'#ffdbac',hair:'#3d2510',zombie:'#4a7a4a',creeper:'#00ff00',coin:'#ffd700',wood:'#7d4d1e',iron:'#ced4da',skeleton:'#d1d1d1',endstone:'#eeeeaa',endstone_spec:'#d8d8a0'};
    ...
    draw('zombie',16,32,ctx=>{ctx.fillStyle=colors.zombie;ctx.fillRect(4,0,8,8);ctx.fillStyle=colors.shirt;ctx.fillRect(4,8,8,12);ctx.fillStyle=colors.pants;ctx.fillRect(4,20,8,12);ctx.fillStyle=colors.zombie;ctx.fillRect(0,8,4,4);ctx.fillRect(12,8,4,4);});
    ...
    platforms=this.physics.add.staticGroup();coins=this.physics.add.group();enemies=this.physics.add.group();arrows=this.physics.add.group();enemyArrows=this.physics.add.group();fireballs=this.physics.add.group();
    ...
    if (nameInput) {
        ...
        nameInput.addEventListener('keydown', sp); nameInput.addEventListener('keyup', sp); nameInput.addEventListener('keypress', sp);
        nameInput.addEventListener('focus', () => { scene.input.keyboard.enabled = false; scene.input.keyboard.removeCapture('W,A,S,D,J,K,L,U,I,ESC'); });
        nameInput.addEventListener('blur', () => { scene.input.keyboard.enabled = true; scene.input.keyboard.addCapture('W,A,S,D,J,K,L,U,I,ESC'); });
    }

    cursors=this.input.keyboard.createCursorKeys();
    keys=this.input.keyboard.addKeys('W,A,S,D,J,K,L,U,I,ESC');
    ...
    this.physics.add.collider(arrows,platforms,a=>a.destroy());this.physics.add.collider(enemyArrows,platforms,a=>a.destroy());
    this.physics.add.collider(fireballs,platforms,f=>f.destroy());
    this.physics.add.overlap(player,coins,(p,c)=>{c.destroy();gameState.coins++;updateHUD();if(gameState.coins>=20)levelClear();});
    this.physics.add.overlap(player,enemies,(p,e)=>{if(!gameState.shieldActive)takeDamage(1);});
    this.physics.add.overlap(player,enemyArrows,(p,a)=>{a.destroy();if(!gameState.shieldActive)takeDamage(1);});
    this.physics.add.overlap(player,fireballs,(p,f)=>{f.destroy();if(!gameState.shieldActive)takeDamage(1);});
...
function generateLevel(scene){
    ...
    if(dragon)dragon.destroy();bossBar.setVisible(false);bossBarBg.setVisible(false);bossText.setVisible(false);
    gameState.coins=0;updateHUD();scene.cameras.main.setBackgroundColor('#87CEEB');
    ...
    for(let i=0;i<15+(gameState.level*3);i++){
        let x=Phaser.Math.Between(800,w-200), r=Math.random(), type='zombie';
        if(r>0.7) type='creeper'; else if(r>0.4) type='skeleton';
        let e=enemies.create(x,400,type);e.type=type;e.setCollideWorldBounds(true);e.health=type==='zombie'?2:1;e.lastShot=0;
    }
}

function startBossLevel(scene){
    if(gameState.playerName === 'UNLUCKY') { die(); return; }
    gameState.isBossLevel=true;gameState.dragonHp=10;gameState.invuln=false;
    platforms.clear(true,true);coins.clear(true,true);enemies.clear(true,true);fireballs.clear(true,true);bossGroup.clear(true,true);enemyArrows.clear(true,true);
    if(dragon)dragon.destroy();scene.cameras.main.setBackgroundColor('#110022');
    bossBarBg.setVisible(true);bossBar.setVisible(true).width=400;bossText.setVisible(true).setText('ENDER DRAGON: 10/10');
    ...
    dragon=bossGroup.create(400,100,'dragon').setScale(3);dragon.body.allowGravity=false;dragon.setCollideWorldBounds(true);dragon.setVelocityX(200);dragon.setBounce(1,1);
    updateHUD();
}

function update(time,delta){
    if(gameState.isPaused||gameState.gameOver||!gameState.isStarted)return;
    if(keys.A.isDown||cursors.left.isDown){player.setVelocityX(-250);player.flipX=true;}
    else if(keys.D.isDown||cursors.right.isDown){player.setVelocityX(250);player.flipX=false;}
    else player.setVelocityX(0);
    const grounded=player.body.touching.down||player.body.blocked.down;
    if((keys.W.isDown||cursors.up.isDown||cursors.space.isDown)&&grounded){player.setVelocityY(-750);player.setScale(1.2,1.8);}
    else if(grounded)player.setScale(1.5);
    ...
    if(Phaser.Input.Keyboard.JustDown(keys.J))swingSword();
    if(Phaser.Input.Keyboard.JustDown(keys.U))usePickaxe();
    if(Phaser.Input.Keyboard.JustDown(keys.I))placeBlock();
    if(keys.K.isDown&&time>gameState.lastFired){fireArrow();gameState.lastFired=time+500;}
    gameState.shieldActive=keys.L.isDown;
    ...
    enemies.getChildren().forEach(e=>{
        ...
        if(d<400){
            if(e.type==='skeleton'){
                e.setVelocityX(d<200? (player.x<e.x?100:-100) : (player.x<e.x?-100:100));
                if(time>e.lastShot){
                    let a=enemyArrows.create(e.x,e.y,'arrow');a.body.allowGravity=false;a.setVelocityX(player.x<e.x?-300:300);a.setFlipX(player.x<e.x);
                    e.lastShot=time+2000;
                }
            } else {
                e.setVelocityX(player.x<e.x?-110:110);
                if(e.type==='creeper'&&d<70)explodeCreeper(e);
            }
        } else e.setVelocityX(0);
    });
    
    if(gameState.isBossLevel&&dragon&&dragon.active){
        dragon.flipX=dragon.body.velocity.x<0;dragon.y=100+Math.sin(time/200)*80;
        if(Math.random()<0.04&&fireballs.countActive(true)<3){let f=fireballs.create(dragon.x,dragon.y+20,'fireball').setScale(1.5);f.body.allowGravity=false;sceneRef.physics.moveToObject(f,player,400);}
    }
    gameState.elapsedTime=(time-gameState.startTime)/1000;updateHUD();
    if(Phaser.Input.Keyboard.JustDown(keys.ESC))pauseGame();
}

function swingSword(){
    sceneRef.cameras.main.shake(100,0.002);let s=sceneRef.add.sprite(player.x+(player.flipX?-30:30),player.y,'swipe').setScale(1.2);s.flipX=player.flipX;
    sceneRef.tweens.add({targets:s,alpha:0,scale:1.8,duration:150,onComplete:()=>s.destroy()});
...
function hitEnemy(e){e.health--;e.setTint(0xff0000);sceneRef.time.delayedCall(100,()=>e.active&&e.clearTint());if(e.health<=0)e.destroy();}

function hitDragon(){
    if(gameState.invuln||!dragon.active)return;
    gameState.invuln=true;gameState.dragonHp--;
    dragon.setTint(0xff0000);sceneRef.cameras.main.flash(150,255,0,0);
    bossBar.width=(gameState.dragonHp/10)*400;bossText.setText(`ENDER DRAGON: ${gameState.dragonHp}/10`);
    sceneRef.time.delayedCall(800,()=>{if(dragon&&dragon.active){dragon.clearTint();gameState.invuln=false;}});
    if(gameState.dragonHp<=0){dragon.destroy();bossText.setVisible(false);bossBar.setVisible(false);bossBarBg.setVisible(false);sceneRef.cameras.main.flash(500,255,255,255);sceneRef.time.delayedCall(500,levelClear);}
}

function usePickaxe(){
    let px=player.x+(player.flipX?-40:40),py=player.y+20,p=sceneRef.add.sprite(px,player.y,'pickaxe').setScale(1.5);p.flipX=player.flipX;
    sceneRef.tweens.add({targets:p,angle:player.flipX?-90:90,duration:100,yoyo:true,onComplete:()=>p.destroy()});
    platforms.getChildren().forEach(b=>{if(Phaser.Math.Distance.Between(px,py,b.x,b.y)<40)b.destroy();});
}

function placeBlock(){let bx=Math.round((player.x+(player.flipX?-48:48))/32)*32,by=Math.round((player.y+16)/32)*32,b=platforms.create(bx,by,gameState.isBossLevel?'endstone':'grass').setScale(2).refreshBody();}

function fireArrow(){
    sceneRef.tweens.add({targets:player,scaleX:1.8,scaleY:1.2,duration:100,yoyo:true});
    let a=arrows.create(player.x,player.y,'arrow');a.body.allowGravity=false;
    if(gameState.isBossLevel){a.setVelocityY(-650);a.setAngle(-90);}else{a.setVelocityX(player.flipX?-650:650);a.setFlipX(player.flipX);}
}

function explodeCreeper(e){e.setTint(0xffffff);sceneRef.time.delayedCall(500,()=>{if(!e.active)return;if(Phaser.Math.Distance.Between(player.x,player.y,e.x,e.y)<130&&!gameState.shieldActive)takeDamage(2);sceneRef.cameras.main.shake(200,0.02);e.destroy();});}
function takeDamage(a){gameState.hp-=a;sceneRef.cameras.main.flash(200,255,0,0);if(gameState.hp<=0)die();updateHUD();}
function updateHUD(){
    let left=`Level: ${gameState.level} | Coins: ${gameState.coins}/20`,hud=document.getElementById('hud');
    if(gameState.isBossLevel){left=`BOSS FIGHT`;hud.style.color='white';hud.style.textShadow='2px 2px #000';}else{hud.style.color='#222';hud.style.textShadow='1px 1px #fff';}
    document.getElementById('stats-left').innerText=left;document.getElementById('stats-right').innerText=`Time: ${(gameState.totalTime+(gameState.isStarted?gameState.elapsedTime:0)).toFixed(1)}s | HP: ${'❤️'.repeat(Math.max(0,gameState.hp))}`;
}
...
function showVictory(){
    ...
    document.getElementById('winner-name').innerText = `WINNER: ${gameState.playerName}`;
    document.getElementById('total-time').innerText=`Final Time: ${gameState.totalTime.toFixed(2)}s`;
    let scores=JSON.parse(localStorage.getItem('mc_scores_v2')||'[]');
    scores.push({name: gameState.playerName, time: gameState.totalTime});
    scores.sort((a,b)=>a.time-b.time);
    localStorage.setItem('mc_scores_v2',JSON.stringify(scores.slice(0,5)));
}
function resetGame(){gameState.level=1;gameState.totalTime=0;gameState.hp=3;gameState.isBossLevel=false;resetLevel();}
function resetLevel(){gameState.gameOver=false;gameState.isPaused=false;gameState.hp=3;gameState.startTime=sceneRef.time.now;gameState.elapsedTime=0;gameState.isStarted=true;sceneRef.physics.resume();if(gameState.level===6)startBossLevel(sceneRef);else generateLevel(sceneRef);}
